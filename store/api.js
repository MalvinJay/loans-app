...
export const actions = {
  ...
  verifyApplication ({ commit }, data) {
    return new Promise((resolve, reject) => {
      const url = 'https://admin.migration.plendify.com/api/verify-id'
  ...
  uploadMedia ({ commit }, data) {
    return new Promise((resolve, reject) => {
      ...
      const url = 'https://admin.migration.plendify.com/api/upload-media'
  ...
  submitApplication ({ state, commit }) {
    return new Promise((resolve, reject) => {
      ...
      if (state.pendingApplication) {
        url = 'https://admin.migration.plendify.com/api/auth/loan-applications'
      } else {
        url = 'https://admin.migration.plendify.com/api/loan-applications'
      }
  ...
  saveApplication ({ state, commit }) {
    return new Promise((resolve, reject) => {
      let url
      if (state.pendingApplication) {
        url = `https://admin.migration.plendify.com/api/unfinished/loan-applications/update/${state.pendingApplication.id}`
      } else {
        url = 'https://admin.migration.plendify.com/api/unfinished/loan-applications/save-continue'
      }
      this.$axios.$post(url, state.general)
        .then((result) => {
          resolve(result)
  ...
  getPendingApplications ({ rootState, state, commit }) {
    return new Promise((resolve, reject) => {
      const url = 'https://admin.migration.plendify.com/api/unfinished/loan-applications'
      this.$axios.$get(url)
        .then((result) => {
